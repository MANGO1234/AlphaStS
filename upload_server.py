...
for ip in ips:
    ...
    p = subprocess.Popen(f'ssh -o StrictHostKeyChecking=no -i {home}\\Downloads\\Test.pem ec2-user@{ip} "bash -c \'killall -9 java; cd ~/alphaStS; git reset --hard; cd agent; mvn install; cd ..; python3 test_dist.py -p > /tmp/alphaStS.log 2>&1 & disown\'"', stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    while p.poll() is None:
        print(p.stdout.readline().decode('ascii'), end='')
    [print(line.decode('ascii'), end='') for line in p.stderr.readlines()]
