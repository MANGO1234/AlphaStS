...
public class CardDefect {
    ...
    public static class SteamBarrier extends Card {
        ...
        public List<Card> getPossibleGeneratedCards(GameProperties properties, List<Card> cards) {
            var c = new ArrayList<Card>();
            for (Card card : cards) {
                if (card.getBaseCard() instanceof SteamBarrier) {
                    for (int i = n - 1; i >= limit; i--) {
                        c.add(card.wrapAfterPlay(new SteamBarrier(i, limit)));
                    }
                }
            }
            return c;
    ...
    public static class SteamBarrierP extends Card {
        ...
        public List<Card> getPossibleGeneratedCards(GameProperties properties, List<Card> cards) {
            ...
            for (Card card : cards) {
                if (card.getBaseCard() instanceof SteamBarrier) {
                    for (int i = n - 1; i >= limit; i--) {
                        c.add(card.wrapAfterPlay(new SteamBarrierP(i, limit)));
                    }
                }
            }
            return c;
    ...
    public static class ForceField extends Card {
        ...
        public List<Card> getPossibleGeneratedCards(GameProperties properties, List<Card> cards) {
            ...
            for (int i = 0; i < cards.size(); i++) {
                if (cards.get(i).getBaseCard() instanceof ForceField) {
                    l.add(cards.get(i).wrap(new ForceField(3)));
                    l.add(cards.get(i).wrap(new ForceField(2)));
                    l.add(cards.get(i).wrap(new ForceField(1)));
                    l.add(cards.get(i).wrap(new ForceField(0)));
                }
            }
            return l;
        }
        ...
        @Override public void gamePropertiesSetup(GameState state) {
            ...
            registerForceFieldCounter(state, this);
        }

        private static void registerForceFieldCounter(GameState state, GameProperties.CounterRegistrant registrant) {
            state.properties.registerCounter("ForceField", registrant, new GameProperties.NetworkInputHandler() {
                @Override public int addToInput(GameState state, float[] input, int idx) {
    ...
    public static class ForceFieldP extends Card {
        ...
        public List<Card> getPossibleGeneratedCards(GameProperties properties, List<Card> cards) {
            ...
            for (int i = 0; i < cards.size(); i++) {
                if (cards.get(i).getBaseCard() instanceof ForceFieldP) {
                    ...
                    l.add(cards.get(i).wrap(new ForceFieldP(0)));
                }
            }
            return l;
        }
        ...
        @Override public void gamePropertiesSetup(GameState state) {
            ...
            ForceField.registerForceFieldCounter(state, this);
        }
    }
    ...
    private static abstract class _MeteorStrikeT extends Card {
        ...
        public GameActionCtx play(GameState state, int idx, int energyUsed) {
            state.playerDoDamageToEnemy(state.getEnemiesForWrite().getForWrite(idx), n + (state.properties.strikeDummy != null && state.properties.strikeDummy.isRelicEnabledInScenario(state.preBattleScenariosChosenIdx) ? 3 : 0));
            state.channelOrb(OrbType.PLASMA);
            state.channelOrb(OrbType.PLASMA);
            state.channelOrb(OrbType.PLASMA);
            return GameActionCtx.PLAY_CARD;
    ...
    private static class _ThunderStrikeT extends Card {
        ...
        public GameActionCtx play(GameState state, int idx, int energyUsed) {
            ...
            for (int i = 0; i < c; i++) {
                ...
                state.playerDoDamageToEnemy(enemy, n + (state.properties.strikeDummy != null && state.properties.strikeDummy.isRelicEnabledInScenario(state.preBattleScenariosChosenIdx) ? 3 : 0));
            }
            return GameActionCtx.PLAY_CARD;
        }
